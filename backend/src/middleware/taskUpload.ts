...
const multerInstance = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (!ALLOWED_MIME_TYPES.has(file.mimetype)) {
      cb(new Error("UNSUPPORTED_TASK_FILE_TYPE"));
      return;
    }
    cb(null, true);
  },
...
function normalizeUploadError(err: unknown): ApiError {
  ...
  if (err instanceof Error && err.message === "UNSUPPORTED_TASK_FILE_TYPE") {
    return ApiError.badRequest("Only JPEG, PNG, WEBP images and PDF files are allowed.");
  }
