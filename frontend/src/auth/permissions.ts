...
export function fullPermissions(): AdminPermissions {
  ...
  for (const key of ADMIN_PERMISSION_KEYS) all[key] = true;
  return all;
}
...
export function firstAllowedAdminPath(permissions: AdminPermissions): string {
  if (permissions.viewDashboard) return "/admin";
  if (permissions.viewAttendance) return "/admin/attendance";
  if (permissions.viewEmployees) return "/admin/employees";
